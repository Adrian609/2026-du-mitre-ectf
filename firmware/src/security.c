...
// Kernel HMAC object for secure operations
KERNEL_DATA static Hmac k_hmac_obj;
...
KERNEL_CODE void init_security()
{
    ...
    // Use kernel HMAC object to reduce stack memory
    if (wc_HmacInit(&k_hmac_obj, NULL, INVALID_DEVID) != 0)
    {
        print_debug("HMAC init failed during security initialization\n");
        return; // Critical failure during init
    }

    if (wc_HmacSetKey(&k_hmac_obj, WC_SHA256, (void *)aes_128_shared_key, AESGCM_KEY_SIZE) != 0)
    {
        print_debug("HMAC setkey failed during security initialization\n");
        wc_HmacFree(&k_hmac_obj);
        return;
    }

    wc_HmacUpdate(&k_hmac_obj, (void *)global_permissions, sizeof(group_permission_t) * MAX_PERMS);
    wc_HmacFinal(&k_hmac_obj, (void *)global_permissions_sig);
    wc_HmacFree(&k_hmac_obj);

    // Clear HMAC object
    memset(&k_hmac_obj, 0, sizeof(Hmac));
    __asm__ volatile("" ::: "memory");
}
 ...
 *
 * @note All input pointers must be non-NULL to prevent memory corruption
 */
KERNEL_CODE void join_bytes(uint8_t *out, ...)
{
    ...
    while ((next_bytes = va_arg(args, uint8_t *)) != NULL)
    {
        ...
        // Security: validate length is reasonable
        if (next_len > 0 && next_len <= 0x10000)
        {
 ...
 *
 * @note Uses chunked processing with internal buffer for memory efficiency;
 *       tag size is AESGCM_TAG_SIZE, iv size is AESGCM_IV_SIZE
 */
                                    ...
                                    bool is_dest_flash)
{
    ...
    if (is_dest_flash)
    {
        // Only erase pages that will actually be written
        uint32_t num_pages = (len + FLASH_PAGE_SIZE - 1) / FLASH_PAGE_SIZE;
        for (uint32_t i = 0; i < num_pages; i++)
        {
            if (flash_simple_erase_page((uint32_t)(out_buffer + (i * FLASH_PAGE_SIZE))) < 0)
                return -1;
        }
        ...
        while (processed < len)
        {
            uint32_t chunk_size = (len - processed < FLASH_PAGE_SIZE) ? (len - processed) : FLASH_PAGE_SIZE;

            memcpy(buffer, in_buffer + processed, chunk_size);

            // TODO: Apply encryption/decryption based on mode
            // For now, write plainly to flash
            if (flash_simple_write((uint32_t)(out_buffer + processed), buffer, chunk_size) < 0)
            {
    ...
    {
        // For SRAM, copy directly and apply transform in-place or use buffer
...
KERNEL_CODE int create_key(uint8_t *secret, uint8_t *nonce, uint32_t nonce_len, uint8_t *key)
{
    ...
    if (nonce_len > 0 && nonce == NULL)
        return INTERNAL_ERR;
    if (AESGCM_KEY_SIZE > (int)sizeof(digest))
        return INTERNAL_ERR;

...
// Some test variables (TODO: remove)
KERNEL_DATA char kdata[] = "KDATA";

KERNEL_BSS char kbss[5];

KERNEL_CONST char ksecret[] = "03ac674216f3e15c761ee1a5e255f067953623c8b388b4459e13f978d7c846f4";

KERNEL_STAGE char kstage[] = "KSTAGE";
...
void print_current_mode(void) {
	...
	uint32_t stack = control & 0x2;

	char str[64];

	sprintf(str,"In %s-mode %s setting with %s stack\n",
...
/** @brief Testing function for MPU protection
 *
 * None of these accesses should work from user code
 * Everything except kernel const write shoud work from
 * kernel code
 *
 * TODO: Remove
*/

volatile uint32_t g_probe_active = 0;
volatile uint32_t g_probe_recover_pc = 0;
volatile uint32_t g_probe_faulted = 0;

void test_mpu_protection(void) {
	/*
	print_debug("Testing kernel data read...\n");
	print_debug(kdata);        // Kernel data read test (Priv allowed, User not)

	print_debug("Testing kernel data write...\n");
	kdata[0] = 'L';            // Kernel data write test (Priv allowed, User not)
	
	print_debug("Testing kernel bss read...\n");
	print_debug(kbss);         // Kernel bss read test (Priv allowed, User not)

	print_debug("Testing kernel bss write...\n");
	kbss[0] = 'L';             // Kernel bss write test (Priv allowed, User not)

	print_debug("Testing kernel const read...\n");
	print_debug(ksecret);      // Kernel const read test (Priv allowed, User not)
	
	print_debug("Testing kernel const write...\n");
	ksecret[0] = 'L';          // Kernel const write test (None allowed)

	print_debug("Testing kernel stage read...\n");
	print_debug(kstage);       // Kernel stage read test (Priv allowed, User not)

	print_debug("Testing kernel stage write...\n");
	// TODO: Kernel stage write test (Priv allowed, User not)

	print_debug("Testing kernel SRAM read...\n");
	uint8_t v = *((uint8_t *)(0x20207FFF)); // Kernel SRAM read test (Priv allowed, User not)
	
	print_debug("Testing kernel SRAM write...\n");
	*((uint8_t *)(0x20207000)) = 0;         // Kernel SRAM write test (Priv allowed, User not)
...
KERNEL_CODE void setup_mpu_layout(void) {
    ...
    */
    MPU->RNR  = 2; 
    MPU->RBAR = 0x00000000;
    //MPU->RASR = (0b00010001 << 24) | (0x03 << 8) | (0x11 << 1) | 1; // TODO: uncomment this line and remove the next
	MPU->RASR = (0b00010011 << 24) | (0x03 << 8) | (0x11 << 1) | 1; // TODO: remove me; this makes files world readable
